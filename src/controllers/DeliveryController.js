...
class Delivery {
  async index(req, res) {
    ...
    try {
      const list = await DeliveryModel.find({ id_user: id });
  ...
  async register(req, res) {
    ...
    try {
      await DeliveryModel.create({ id_user: id, delivery: req.body });
  ...
  async edit(req, res) {
    ...
    if (!id && typeof id !== 'string') return res.status(404).json({ errors: 'Numero do ID inválido.' });
    try {
      await DeliveryModel.findByIdAndUpdate(id, { delivery: req.body });
